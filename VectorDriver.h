...
void print_even(const vector<short> &data) //prints off only the even elements in the vector
{
	if (data.empty()) 
	{
		cout << "<empty>";
...
{
	std::vector<short>::const_iterator iter = data.begin();//interates through the vector and compares each element to the short value
	while (iter != data.end()) 
	{
		if (*iter == value)
		{
